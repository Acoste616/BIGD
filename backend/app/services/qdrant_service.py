...
class QdrantService:
            ...
            if self.collection_name not in collection_names:
                ...
                self.client.create_collection(
                    ...
                    vectors_config=VectorParams(
                        size=384, distance=Distance.COSINE  # Rozmiar wektora dla MiniLM
                    ),
                )
        ...
        try:
            ...
            for knowledge_data in knowledge_items:
                ...
                payload = {
                    ...
                    or f"Wskazówka {datetime.now().strftime('%Y-%m-%d %H:%M')}",
            ...
            if knowledge_type:
                must_conditions.append(
                    models.FieldCondition(
                        key="knowledge_type", match=models.MatchValue(value=knowledge_type)
                    )
                )
            if archetype:
                must_conditions.append(
                    models.FieldCondition(key="archetype", match=models.MatchValue(value=archetype))
                )
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "unhealthy", "error": str(e), "collection_name": self.collection_name}


try:
    qdrant_service = QdrantService()
except Exception as exc:  # pragma: no cover - executed only when service is unavailable
    logger.warning("QdrantService initialisation failed: %s", exc)
    qdrant_service = None
