...
def initialize_ai_service(qdrant_service: QdrantService) -> AIServiceUnified:
    ...
    """
    global ai_service_unified, ai_service
    
    try:
        if ai_service_unified is None:
            ai_service_unified = AIServiceUnified(qdrant_service)
            # Update all global aliases for backward compatibility
            globals()['ai_service'] = ai_service_unified
...
# Export get_ai_service for compatibility
def get_ai_service():
    """Backward compatibility function"""
    return ai_service_unified

# Additional backward compatibility for old imports  
# Note: ai_service is updated dynamically in initialize_ai_service()
ai_service = ai_service_unified