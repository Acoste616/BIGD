...
E2E_TESTS_ENABLED = os.getenv("RUN_E2E_TESTS") == "1"
pytestmark = pytest.mark.skipif(
    not E2E_TESTS_ENABLED,
    reason="E2E tests require the full stack to be running.",
)

# Konfiguracja testów
BASE_URL = "http://localhost:8000/api/v1"
KNOWLEDGE_BASE_PATH = (
    Path(__file__).parent / "knowledge_base_pl.json"
)  # Ścieżka względna do pliku testu
...
TEST_SESSION_DATA = {"session_type": "consultation", "notes": "Testowa sesja E2E"}
...
async def test_health_checks():
    ...
    async with httpx.AsyncClient(base_url=BASE_URL) as client:
        ...
        assert health_data["application"]["status"] in [
            "healthy",
            "degraded",
        ...
        assert (
            "db_status" in db_health or "status" in db_health
        ), "DB health response missing status"
        ...
        assert (
            response_qdrant.status_code == 200
        ), f"Qdrant health check failed: {response_qdrant.status_code}"
...
async def test_knowledge_base_import_and_stats():
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=60.0) as client:
        ...
        assert response_import.status_code in [
            200,
            201,
        ], f"Import failed: {response_import.status_code} - {response_import.text}"
        ...
        assert import_result["success_count"] == len(
            knowledge_batch
        ), f"Import incomplete: {import_result['success_count']} != {len(knowledge_batch)}"
        assert (
            import_result["error_count"] == 0
        ), f"Import errors occurred: {import_result['error_count']}"
...
async def test_full_user_workflow_with_rag():
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=30.0) as client:
        ...
        assert (
            response_client.status_code == 201
        ), f"Client creation failed: {response_client.status_code} - {response_client.text}"
        ...
        response_session = await client.post(
            f"/clients/{client_id}/sessions/", json=TEST_SESSION_DATA
        )
        assert (
            response_session.status_code == 201
        ), f"Session creation failed: {response_session.status_code} - {response_session.text}"
        ...
        assert (
            session_data["session_type"] == TEST_SESSION_DATA["session_type"]
        ...
        response_interaction = await client.post(
            f"/sessions/{session_id}/interactions/", json=TEST_INTERACTION_DATA
        )
        assert (
            response_interaction.status_code == 201
        ), f"Interaction creation failed: {response_interaction.status_code} - {response_interaction.text}"
        ...
        full_response_text = (
            ai_response.get("quick_response", "") + " " + ai_response.get("main_analysis", "")
        ).lower()
        ...
        found_indicators = [
            indicator for indicator in rag_indicators if indicator in full_response_text
        ]
        ...
        ), f"Zbyt mało wskaźników RAG: {found_indicators}. Odpowiedź może nie korzystać z bazy wiedzy."
